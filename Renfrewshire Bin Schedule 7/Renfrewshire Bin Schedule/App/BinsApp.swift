...
func listenForTransactions() {
        Task {
            // Listen indefinitely for transaction updates.
            for await transactionResult in Transaction.updates {
                switch transactionResult {
                case .verified(let transaction):
                    // Process the transaction as needed and then finish it.
                    print("Transaction verified: \(transaction)")
                    await transaction.finish()
                case .unverified(_, let error):
                    print("Transaction unverified with error: \(error.localizedDescription)")
                }
            }
        }
    }
